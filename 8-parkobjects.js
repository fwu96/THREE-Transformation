...
export class GrCarousel extends GrObject {
    ...
    constructor(params={}) {
		...
		// Store the y position of each horse (they are different)
		let hypos = [];
		for (let i = 13; i < platform_group.children.length; i++) {
			hypos.push(platform_group.children[i].position.y);
		}
		// rotating the carousel and horse go up and down on each poles
 		this.advance = function(delta, timeOfDay) { 
			carousel.rotateY(0.001*delta);
			let theta = performance.now() / 1000;
			let idx = 0;
			// each horse has a random moving speed and their own starting position
			for (let i = 13; i < platform_group.children.length; i++) {
				idx = i - 13;
				platform_group.children[i].position.y = hypos[idx] - (0.5 * Math.sin(theta * (idx + 1) * 0.15));
			}
		}
		// Function for create the shape of horse
		function Horse() {
			let ohorse = new T.Group();
			let Colors = ["#f48FB1", "#F5F5F5", "#FFCCBC"];
			let rc_idx = Math.floor(Math.random() * 3);
    		let mhorse = new T.MeshPhongMaterial({color: Colors[rc_idx]});
    		let geoLeg = new T.BoxGeometry(0.2, 0.7, 0.2);
    		let l1 = new T.Mesh(geoLeg, mhorse);
    		l1.position.set(0.2, 0, 0.3);
    		ohorse.add(l1);
    		let l2 = new T.Mesh(geoLeg, mhorse);
    		l2.position.set(-0.2, 0, 0.3);
    		ohorse.add(l2);
    		let l3 = new T.Mesh(geoLeg, mhorse);
    		l3.position.set(0.2, 0, -0.3);
    		ohorse.add(l3);
    		let l4 = new T.Mesh(geoLeg, mhorse);
    		l4.position.set(-0.2, 0, -0.3);
    		ohorse.add(l4);
    		let geoBody = new T.BoxGeometry(0.7, 0.4, 1);
    		let body = new T.Mesh(geoBody, mhorse);
    		body.position.y = 0.5;
    		ohorse.add(body);
    		let geoNeck = new T.BoxGeometry(0.6, 0.4, 0.3);
    		let neck = new T.Mesh(geoNeck, mhorse);
    		neck.rotateY(Math.PI / 2);
    		neck.rotateZ(-45 * Math.PI / 180);
    		neck.position.set(0, 0.8, 0.5);
    		ohorse.add(neck);
    		let geoHead = new T.BoxGeometry(0.4, 0.2, 0.3);
    		let head = new T.Mesh(geoHead, mhorse);
    		head.rotateY(Math.PI / 2);
    		head.rotateZ(45 * Math.PI / 180);
    		head.position.set(0, 1, 0.7);
    		ohorse.add(head);
    		let geoNose = new T.BoxGeometry(0.3, 0.25, 0.25);
    		let nose = new T.Mesh(geoNose, mhorse);
    		nose.rotateX(45 * Math.PI / 180);
    		nose.position.set(0, 0.8, 0.85);
    		ohorse.add(nose);
    		let geoTail = new T.BoxGeometry(0.2, 0.5, 0.2);
    		let tail = new T.Mesh(geoTail, mhorse);
