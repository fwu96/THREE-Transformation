...
export class GrBackhoe extends GrObject {
	...
	constructor(params = {}) {
		...
		// The base of backhoe
		let sbase = new T.Shape();
		let base_group = new T.Group();
    	sbase.moveTo(-1, 0);
    	sbase.lineTo(-1.2, 0.2);
    	sbase.lineTo(-1.2, 0.4);
		sbase.lineTo(-1, 0.6);
		sbase.lineTo(-0.5, 0.6);
		sbase.lineTo(-0.5, 0.7);
		sbase.lineTo(0.5, 0.7);
		sbase.lineTo(0.5, 0.6);
    	sbase.lineTo(1, 0.6);
    	sbase.lineTo(1.2, 0.4);
    	...
    	sbase.lineTo(1, 0);
    	sbase.lineTo(-1, 0);
    	let gbase = new T.ExtrudeGeometry(sbase, exSettings);
    	let mbodyMain = new T.MeshStandardMaterial({color: "#FFD54F", metalness: 0.5, roughness: 0.7});
		let base = new T.Mesh(gbase, mbodyMain);
		base_group.add(base);
		backhoe.add(base_group);
		// Driver room of the backhoe
    	...
    	sbody.lineTo(0, 0.6);
    	sbody.lineTo(0, 1.2);
    	sbody.lineTo(0.2, 1.4);
    	sbody.lineTo(0.8, 1.4);
    	sbody.lineTo(1, 1.2);
    	sbody.lineTo(1, 0);
    	let gbody = new T.ExtrudeGeometry(sbody, exSettings);
		let body = new T.Mesh(gbody, mbodyMain);
		cab_group.add(body);
		// the front-dozer of the backhoe
		let sdozer = new T.Shape();
		let frontDozerGroup = new T.Group();
		frontDozerGroup.position.set(-1.5, 0.6, 0);
		base_group.add(frontDozerGroup);
    	sdozer.moveTo(0, 0);
    	sdozer.lineTo(0.3, -0.2);
    	sdozer.lineTo(0.3, -0.4);
    	sdozer.lineTo(0, -0.6);
    	sdozer.lineTo(-0.5, -0.6);
    	sdozer.lineTo(0, 0);
		let gdozer = new T.ExtrudeGeometry(sdozer, exSettings);
		let mdozer = new T.MeshStandardMaterial({color: "#8D6E63", metalness: 0.6, roughness: 0.3});
		let dozer = new T.Mesh(gdozer, mdozer);
		frontDozerGroup.add(dozer);
		// the station that connect arm at the back
		let sbackArmSt = new T.Shape();
		let armStGroup = new T.Group();
		armStGroup.translateY(0.6);
		cab_group.add(armStGroup);
    	sbackArmSt.moveTo(1, 0);
    	sbackArmSt.lineTo(1, 0.2);
    	sbackArmSt.lineTo(1.3, 0.2);
    	sbackArmSt.lineTo(1.3, 0);
    	sbackArmSt.lineTo(1, 0);
		let garmSt = new T.ExtrudeGeometry(sbackArmSt, exSettings);
		let mbodySup = new T.MeshStandardMaterial({color: "#888888", metalness: 0.6, roughness: 0.3});
    	let armSt = new T.Mesh(garmSt, mbodySup);
    	armStGroup.add(armSt);
		// arm at the back
		let sbackArm = new T.Shape();
		let armGroup = new T.Group();
		armGroup.position.set(1.3, 0.55, 0);
		armStGroup.add(armGroup);
    	sbackArm.moveTo(0, 0);
    	sbackArm.lineTo(0.5, 1);
    	sbackArm.lineTo(0.5, 0.8);
    	sbackArm.lineTo(0, -0.6);
    	let gbackArm = new T.ExtrudeGeometry(sbackArm, exSettings);
    	let backArm = new T.Mesh(gbackArm, mbodySup);
    	armGroup.add(backArm);
		// forearm of the arm
		let sforeArm = new T.Shape();
		let foreArmGroup = new T.Group();
		foreArmGroup.position.set(0.5, 0.75, 0);
		armGroup.add(foreArmGroup);
    	sforeArm.moveTo(0, 0);
    	sforeArm.lineTo(1.5, 0);
		sforeArm.lineTo(1.6, 0.2);
    	sforeArm.lineTo(0, 0.2);
    	let gforeArm = new T.ExtrudeGeometry(sforeArm, exSettings);
    	let foreArm = new T.Mesh(gforeArm, mbodySup);
    	foreArmGroup.add(foreArm);
		// dozer at the back-arm
		let sbackDozer = new T.Shape();
		let backDozerGroup = new T.Group();
		backDozerGroup.position.set(1.6, 0.2, 0);
		foreArmGroup.add(backDozerGroup);
		sbackDozer.moveTo(0, 0);
		sbackDozer.lineTo(0.8, 0);
		sbackDozer.lineTo(1, -0.2);
		sbackDozer.lineTo(1, -0.5);
		sbackDozer.lineTo(0.8, -0.8);
		sbackDozer.lineTo(1, -0.8);
		sbackDozer.lineTo(0, 0);
    	let gbackDozer = new T.ExtrudeGeometry(sbackDozer, exSettings);
    	let backDozer = new T.Mesh(gbackDozer, mdozer);
    	backDozerGroup.add(backDozer);
		// Parameter of the sliders
		super(`Backhoe-${backHoeObCtr++}`, backhoe,
			  [ ['x',-7,7,0],
				['z',-6,6,0],
				['theta',0,360,0],
				['spin',0,360,0],
				['front dozer rotate', 0, 20, 0],
				['back arm rotate',-30,50,0],
				['back forearm rotate',-40,90,0],
				['back bucket rotate',-90,45,0]
			  ]);
		this.whole_obj = backhoe;
		this.body = cab_group;
		this.arm = armGroup;
		this.backForeArm = foreArmGroup;
		this.backDozer = backDozerGroup;
		this.dozer = frontDozerGroup;
		// translations of the backhoe
	...
	// silder control
	update(paramValues) {
		this.whole_obj.position.x = paramValues[0];
		this.whole_obj.position.z = paramValues[1];
		this.whole_obj.rotation.y = degreesToRadians(paramValues[2]);
		this.body.rotation.y = degreesToRadians(paramValues[3]);
		this.dozer.rotation.z = -degreesToRadians(paramValues[4]);
		this.arm.rotation.z = degreesToRadians(-paramValues[5]);
		this.backForeArm.rotation.z = degreesToRadians(paramValues[6]);
		this.backDozer.rotation.z = degreesToRadians(paramValues[7]);
	}
}